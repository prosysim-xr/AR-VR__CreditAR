...
public class S2_CamPlayAndToggle2 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (currentScene.name == "S2_Scan")
        {
            ...
            processingText.text = "";
            scanCapturedText = GameObject.FindGameObjectWithTag("scanCaptured").GetComponent<Text>();
            scanCapturedText.text = "";
        }
        //if ((Screen.height / Screen.width) >= 0)
        //{
        //    playCam_GameObject.transform.localScale = new Vector3(Screen.height, Screen.height, 1);//change the size of quad
            
        //}
        //else
        //{
        //    playCam_GameObject.transform.localScale = new Vector3(Screen.width, Screen.width, 1);//change the size of quad
        //    PlayCameraStart();
        //}
        PlayCameraStart();
    }
    ...
    private void PlayCameraStart()
    {
        ...
        //
        playCam_GameObject.GetComponent<Renderer>().material.mainTexture = UtilityHelper.backCam;//apply texture on the quad
    ...
    private void Update()
    {
        ...
        if (isSnapTaken)
        {
            scanCapturedText.text = "Scan Done";
            isSnapTaken = false;
            Conversion();
        }
    }
    ...
    IEnumerator PostRequest(string url, string json)
    {
        ...
        else
        {
            ...
            {
                print("wrong Card! Please Try Again");
                processingText.text = "wrong Card! Please Try Again";//todo
                scanCapturedText.text = "";
            }
